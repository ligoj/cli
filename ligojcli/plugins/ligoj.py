...
def configure(subparser_service):
    ...
    parser_action.add_argument(
        ...
        "-w",
        type=int,
        help="Wait for status is available  up to given duration in seconds, -1 for unlimited, 0 for immediate return",
        default=0,
    )
    ...
    parser_action.add_argument(
        "--api_key_name",
        "-k",
        help="Create and return an API key with the given name",
        required=False,
    )
    ...
    parser_action.add_argument(
        "--password",
        "-p",
        help="User password for login authentication. Only returned cookies are saved in home",
        required=False,
        default="",
    )
    ...
    parser_action.add_argument(
        "--expiration",
        "-e",
        help="Token expiration date or duration (e.g. 1d, 2w, 3m, 4y)",
        required=False,
        default="",
    )
    parser_action.add_argument(
        "--save",
        "-s",
        help="When set, token is instead output in credentials file",
        default=False,
        action="store_true",
    )
    ...
    parser_action.add_argument(
        "--version",
        "-v",
        help="Maven artifact-id of the plugin. 'LATEST' to check the repository.",
        required=False,
    )
    parser_action.add_argument(
        "--repository",
        "-r",
        help="Maven repository manager such as 'central' and 'nexus'.",
        default="central",
    )
    parser_action.add_argument(
        "--javadoc",
        "-j",
        help="Install Javadoc with this plugin for OpenAPI completeness",
        action="store_true",
        default=False,
    )
    parser_action.add_argument(
    ...
    parser_action.add_argument(
        ...
        "-w",
        type=int,
        help="Wait for the status is available up to given duration in seconds, -1 for unlimited, 0 for immediate return",
        default=0,
    )
    ...
    parser_action.add_argument(
        "--parameters-mode",
        "-m",
        choices=["all", "create", "link", "none"],
        help="Retrieve parameters options. By default, no parameters are returned",
        default="none",
    )
    parser_action.add_argument(
        "--parameters-output",
        "-c",
        choices=["full", "list", "map"],
        default="list",
        help="Parameter output mode and structure",
    )
    parser_action.add_argument(
        "--parameters-secured",
        "-s",
        action="store_true",
        help="Return secured parameter values",
        default=False,
    )
    ...
    parser_action.add_argument(
        "--depth",
        "-D",
        help="Refinement depth. 0 is the top level. -1 for any depths",
        type=int,
        default=-1,
    )
    ...
    parser_action.add_argument(
        "--id",
        "-i",
        help="[deprecated, use 'upsert'] Node identifier. Related plugin must be previously installed",
    )
    ...
    parser_action.add_argument(
        ...
        "-s",
        dest="service_id",
        help="Ligoj service identifier, ie. `service:id`",
        required=False,
    )
    ...
    parser_action.add_argument(
        "--with-data",
        "-d",
        help="When set, remote data created by the subscription is also deleted",
        action="store_true",
    )
    ...
    parser_action.add_argument(
        "--with-data",
        "-w",
        help="When activated, subscriptions are deleted along with data they have generated",
        action="store_true",
        default=False,
    )
    ...
    parser_action.add_argument(
        "--team-leader",
        "-t",
        help="Assigned team leader, by default, the current API user",
        required=False,
    )
    ...
    # delegate node
    subparser_action = subparser_service.add_parser(
        "delegate-node", help="Delegate node operations"
    ).add_subparsers(title="action", help="Action", dest="action")
    ...
    # delegate org
    subparser_action = subparser_service.add_parser(
        "delegate-org", help="Delegate organization operations"
    ).add_subparsers(title="action", help="Action", dest="action")
    parser_action = subparser_action.add_parser(
    ...
    parser_action.add_argument(
        "--system",
        "-S",
        help="When true, the value is also stored at system level",
        action="store_true",
    )
    ...
    parser_action.add_argument(
        ...
        "-d",
        required=True,
        help="Working directory of executed command. Must not contain spaces",
    )
    parser_action.add_argument(
        "--command",
        "-c",
        required=True,
        help="Command to execute, split by ` ` char to separate program from its arguments. Must be allowed by `ligoj.hook.path` configuration.",
    )
    parser_action.add_argument(
        "--match",
        "-m",
        required=True,
        help='Hook JSON structure. Currently supports only path and optionally method filtering. ie. {"path": "rest/path/to", "method": "GET"}',
    )
    parser_action.add_argument(
        "--inject",
        help='Can relate to any configuration name supported by the "configuration get" command. Decrypted as needed.',
        nargs="*",
        default=[],
    )
    parser_action.add_argument(
    ...
    parser_action.add_argument(
        "--path",
        "-p",
        help="File path to delete. Must be allowed by `ligoj.file.path` configuration.",
    )
    parser_action = subparser_action.add_parser("get", help="Return file content")
    parser_action.add_argument(
        "--path",
        "-p",
        help="File path to return. Must be allowed by `ligoj.file.path` configuration.",
    )
...
def execute_action(service, action, _, args):
    if service == "session":
        ...
        if action in ["create", "upsert"]:
            ...
            node_create_definition = utils.load_json_from_url_or_file_with_interpolation(
                utils.not_none(args["from"], "node file/URL definition"),
        ...
        if action == "create":
            return project_create(
                args.get("team_leader", ligoj_api_user),
                args.get("pkey", args["name"]),
                args["name"],
        ...
        if action == "set":
            return configuration_set(
                utils.not_none(args.get("id"), "configuration name"),
                utils.not_none(args.get("value"), "configuration value"),
        ...
        if action == "create":
            return token_create(
                utils.not_none(args.get("id"), "token name"),
                args.get("expiration", ""),
                args.get("save", False),
            )
        if action == "list":
        ...
        if action == "create":
            ...
            return system_role_create(
                ...
                utils.not_none(args.get("ui"), "UI patterns"),
            )
        if action == "list":
        ...
        if action == "upsert":
            return system_user_upsert(
                utils.not_none(args.get("id"), "user id"),
                args.get("roles", []),
                args.get("api_key_name"),
            )
        if action == "list":
...
def call_api(method, url, **kwargs):
    ...
    if ligoj_api_user is not None and ligoj_api_key is not None:
        ...
        if ligoj_api_run_as_user is not None and ligoj_api_key is not None:
            headers = {
                "x-api-user": ligoj_api_run_as_user,
                "x-api-via-user": ligoj_api_user,
                "x-api-key": ligoj_api_key,
            } | headers
        else:
    ...
    response = utils.call_rest_api(
        method,
        "ligoj",
        f"{ligoj_endpoint}{'/' if url.startswith('/') else '/rest/'}",
        url.removeprefix("/"),
        None,
        kwargs,
    )
...
def plugin_install_internal(
    ...
    if from_location:
        ...
        with open(upload_file, "rb") as file:
            call_api(
                "PUT",
                "system/plugin/upload",
                files={"plugin-file": file},
                data={"plugin-id": artifact_id, "plugin-version": target_version},
            )
        utils.delete_temp_file_from(from_location, upload_file)
    else:
        call_api(
            "POST",
            f"system/plugin/{artifact_id}/{target_version}",
            params={"repository": repository, "javadoc": javadoc},
        )
...
def configuration_get(name: str | None):
    ...
    response = call_api(
        "GET",
        f"system/configuration/{urllib.parse.quote(name, safe='')}",
        headers={"Accept": "text"},
    )
...
def hook_upsert(
    ...
    try:
        ...
        if match_obj.get("method", "GET") not in [
            "DELETE",
            "GET",
            "HEAD",
            "OPTIONS",
...
def file_put(from_location: str, to_path: str, executable: bool = False):
    ...
    with open(upload_file, "rb") as file:
        call_api(
            ...
            "system/file",
            files={"content": file},
            data={"path": to_path, "executable": executable},
        )
    utils.delete_temp_file_from(from_location, upload_file)
    return False
...
def session_login_password(password: str):
    ...
    result = utils.call_rest_api(
        ...
        None,
        {
            "params": {"username": ligoj_api_user, "password": password},
...
def node_list(
    ...
    mode: str | None,
    depth: int,
    parameters_mode: str | None = None,
    parameters_output: str | None = None,
    return_secured_parameters=False,
):
    response = call_api(
        "GET",
        "node",
...
def node_get_parameters(
    ...
    if parameters_mode is not None or parameters_mode == "":
        ...
        for some_node in nodes:
            ...
            p_response = call_api(
                "GET",
                f"node/{some_node_id}/parameter-value/{parameters_mode}{secured_path}",
                ignore_error=True,
                ignore_output=True,
            )
        ...
        if get_parameter_type(parameter):
            ...
            parameters_as_dict[parameter_id] = parameter.get(
                ...
                parameter.get(
                    ...
                    parameter.get(
                        "integer",
                        parameter.get("date", parameter.get("index", parameter.get("bool"))),
                    ),
                ),
            )
