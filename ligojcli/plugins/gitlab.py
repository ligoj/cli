...
def parse_remote_args(args):
    ...
    gitlab_base_group = utils.get_config(
        args, "gitlab_base_group", "GITLAB_BASE_GROUP", DEFAULT_GITLAB_BASE_GROUP
    )
    gitlab_project_group_prefix = utils.get_config(
        args,
        "gitlab_project_group_prefix",
        "GITLAB_PROJECT_GROUP_PREFIX",
        DEFAULT_GITLAB_PROJECT_GROUP_PREFIX,
    )
    gitlab_project_subgroup_prefix = utils.get_config(
        args,
        "gitlab_project_subgroup_prefix",
        "GITLAB_PROJECT_SUBGROUP_PREFIX",
        DEFAULT_GITLAB_PROJECT_SUBGROUP_PREFIX,
    )
    gitlab_wrapper_group = utils.get_config(
        args, "gitlab_wrapper_group", "GITLAB_WRAPPER_GROUP", None
    )
    gitlab_wrapper_group_name = utils.get_config(
        args, "gitlab_wrapper_group_name", "GITLAB_WRAPPER_GROUP_NAME", None
    )
...
def call_gitlab_api(method, url, **kwargs):
    response = utils.call_rest_api(
        method,
        "gitlab",
        f"{gitlab_endpoint}/api/v4",
        url,
        None,
        kwargs | {"headers": {"PRIVATE-TOKEN": gitlab_token}},
    )
...
def gitlab_list_sub_groups(parent_path: str, data):
    ...
    return call_gitlab_api(
        "GET", f"/groups/{urllib.parse.quote(path_no_slash, safe='')}/subgroups", data=data
    )
...
def gitlab_add_group_member(
    path_or_id: int | str | None, user_id: str, access_level: int = 50, ignore_error=True
):
    path_no_slash = re.sub(r"^/", "", str(path_or_id))
    return call_gitlab_api(
        "POST",
        f"/groups/{urllib.parse.quote(path_no_slash, safe='')}/members",
...
def gitlab_create_group(parent_id: int | str | None, path: str, definition, avatar: bool):
    ...
    response = call_gitlab_api(
        ...
        | {
            ...
            "path": path,
            "parent_id": parent_id
            if not parent_id or type(parent_id) is int
            else urllib.parse.quote(path_no_slash, safe=""),
        },
    )
    if avatar:
        # Also upload the avatar
        call_gitlab_api(
            "PUT", f"/groups/{response['id']}", files={"avatar": open("customize/avatar.png", "rb")}
        )
    return response
...
def gitlab_delete_group(path_or_id: int | str, ignore_error=True):
    ...
    path_no_slash = re.sub(r"^/", "", str(path_or_id))
    call_gitlab_api(
        "DELETE", f"/groups/{urllib.parse.quote(path_no_slash, safe='')}", ignore_error=ignore_error
    )


def gitlab_create_project_roles(
    project_group_path: str,
    project_key: str,
    ...
    gitlab_project_full_path = (
        ...
        if base_group_is_root
        else f"{base_group_path}/{gitlab_project_group_path}"
    )
    ...
    if project_group:
        ...
        if update_mode == utils.UPDATE_MODE_ONCE:
            utils.info(
                f"[gitlab] Project group '{project_group}'({project_group.get('id')}) already exists and update mode is '{utils.UPDATE_MODE_ONCE}', skip sub-groups creation"
            )
            return
    else:
        if base_group:
            utils.info(
                f"[gitlab] Create project group '{gitlab_project_group_path}' inside '{base_group['full_path']}'"
            )  # pylint: disable=unsubscriptable-object
            project_group = gitlab_create_group(
                base_group["id"], gitlab_project_group_path, {"name": project_key}, False
            )  # pylint: disable=unsubscriptable-object
        else:
            utils.info(f"[gitlab] Create project group '{gitlab_project_group_path}' at root level")
            project_group = gitlab_create_group(
                None, gitlab_project_group_path, {"name": project_key}, False
            )
    ...
    wrapper_group_path = (
        ...
        if gitlab_wrapper_group and len(gitlab_wrapper_group) and gitlab_wrapper_group != "/"
        else None
    )
    wrapper_group_name = (
        gitlab_wrapper_group_name
        if wrapper_group_path and gitlab_wrapper_group_name and len(gitlab_wrapper_group_name)
        else wrapper_group_path
    )
    utils.info(
    ...
    if wrapper_group_path:
        ...
        if not project_wrapper_group:
            utils.info(
                f"[gitlab] Create project wrapper group '{wrapper_group_path}' inside '{gitlab_project_full_path}'"
            )
            project_wrapper_group = gitlab_create_group(
                project_group.get("id") or None,
                wrapper_group_path,
                {"name": wrapper_group_name, "project_creation_level": "noone"},
                True,
            )
    ...
    for sub_group in definition.get("roles", {}).keys():
        ...
        utils.info(
            f"[gitlab] Search project sub groups matching '{gitlab_subgroup_path}' inside '{project_wrapper_group_full_path}' ..."
        )
        gitlab_subgroup = gitlab_get_group(
            f"{project_wrapper_group_full_path}/{gitlab_subgroup_path}", ignore_error=True
        )

        if gitlab_subgroup:
            utils.info(
                f"[gitlab] Project sub group '{gitlab_subgroup_path}' already exists: '{gitlab_subgroup.get('id')}'"
            )
        else:
            utils.info(
                f"[gitlab] Project sub group '{gitlab_subgroup_path}' does not exist, create it"
            )
            gitlab_subgroup = gitlab_create_group(
                project_wrapper_group.get("id"),
                gitlab_subgroup_path,
                {"name": gitlab_subgroup_name, "project_creation_level": "noone"},
                True,
            )
        ...
        if sub_group == "admin":
            ...
            team_leader = (
                ligoj.user_find_by_id_or_mail(on_behalf_of) if on_behalf_of else ligoj.whoami()
            )
            ...
            if team_leader_id:
                ...
                utils.warn(
                    f"[gitlab] User '{team_leader}' cannot be found, and cannot be added to group '{gitlab_subgroup['full_path']}'"
                )  # pylint: disable=unsubscriptable-object


def gitlab_delete_project_roles(
    project_group_path: str, groups_by_name: dict[str, str], definition, with_data: bool
):
    ...
    if wrapper_group_path:
        ...
        for sub_group in definition.get("roles", {}).keys():
            ...
            gitlab_subgroup_path = f"{gitlab_project_subgroup_prefix}{ldap_group_name}"
            gitlab_delete_group(
                f"{gitlab_project_full_path}/{gitlab_subgroup_path}", ignore_error=True
            )
