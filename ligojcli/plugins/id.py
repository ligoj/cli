...
from ligojcli.plugins import utils, ligoj
...
def execute_action(service, action, _, args):
    if service == "id:delegate-node":
        ...
        if action == "create":
            return container_scope_create(utils.not_none(args.get("name"), "name"), utils.not_none(args.get("type"), "type"), utils.not_none(args.get("dn"), "dn"))
        if action == "delete":
            if args.get("id") is None and (args.get("name") is None or args.get("type") is None):
        ...
        if action == "create":
            return ou_create(utils.not_none(args.get("name"), "name"), utils.not_none(args.get("parent_dn"), "parent-dn"))
        if action == "delete":
...
def delegate_node_create(node_id: str, can_subscribe: bool, can_write: bool, can_admin: bool, receiver: str, receiver_type: str) -> int:
    return ligoj.call_api(
        "POST", "node/delegate", data={"name": node_id, "canSubscribe": can_subscribe, "canWrite": can_write, "canAdmin": can_admin, "receiver": receiver, "receiverType": receiver_type}
    )


def user_reset_password(user):
    utils.info(f"[ligoj] Reset password of user '{user}' ...")
    return ligoj.call_api("PUT", f"service/id/user/{urllib.parse.quote(user, safe='')}/reset", headers={"Accept": "text/plain"}).text
...
def group_import(csv_file):
    ...
    return ligoj.call_api("POST", "service/id/group/batch", data={"csv-file": csv_file, "encoding": "UTF-8", "columns": ["name", "scope", "parent", "department", "owner", "assistant"]}).json()
...
def container_scope_get_id(name_or_id: str | int, container_type: str | None, required: bool = True) -> int:
    ...
    response = ligoj.call_api("GET", f"service/id/container-scope/name/{urllib.parse.quote(name_or_id, safe='')}/{container_type}", ignore_error=True)
    if not response:
        if required:
...
def container_scope_get_by_name(name: str, container_type: str):
    ...
    return ligoj.call_api("GET", f"service/id/container-scope/name/{urllib.parse.quote(name, safe='')}/{container_type}").json()
...
def container_scope_create(name: str, container_type: str, dn: str) -> int:
    ...
    container_response = ligoj.call_api("GET", f"service/id/container-scope/name/{urllib.parse.quote(name, safe='')}/{container_type}", ignore_error=True)
    if container_response is not None:
        ...
        if container_response.json()["dn"] == dn:
            ...
            ligoj.call_api("PUT", "service/id/container-scope", data={"id": existing_id, "dn": dn, "name": name, "type": container_type})
        return existing_id
...
def company_create(name: str | int, container_scope: str | int, **kwargs):
    ...
    return ligoj.call_api("POST", "service/id/company", data=kwargs.get("data", {}) | {"name": name, "scope": container_scope}, ignore_error=kwargs.get("ignore_error", False))
...
def ou_create(name: str, parent_dn: str, **kwargs):
    ...
    try:
        ou_response = ligoj.call_api("POST", "service/id/company", data={"name": name, "scope": container_scope_id}, ignore_error=kwargs.get("ignore_error", False))
    finally:
...
def group_create(name: str, container_scope_name_or_id: str | int, parent_name: str | None = None):
    ...
    return ligoj.call_api("POST", "service/id/group", data={"name": name, "scope": container_scope_id, "parent": parent_name})


def project_get(project_key_or_id, headers=None):
    response = ligoj.call_api("GET", f"project/{project_key_or_id}", ignore_error=True, ignore_output=True, headers=headers)
    if response is None:
        return None
...
def user_add_to_group(user, group):
    ...
    if group not in user_details["groups"]:
        return ligoj.call_api("PUT", f"service/id/user/{urllib.parse.quote(user, safe='')}/group/{urllib.parse.quote(group, safe='')}")
...
def user_remove_from_group(user, group):
    ...
    if group in user_details["groups"]:
        return ligoj.call_api("DELETE", f"service/id/user/{urllib.parse.quote(user, safe='')}/group/{urllib.parse.quote(group, safe='')}")
