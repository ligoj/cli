...
def configure(subparser_service):
    ...
    parser_plugin = subparser_action.add_parser(
        "plugin",
        help="Ligoj plugin dev helpers ('create', 'build', 'renovate', 'deploy', 'pull')",
    )
    ...
    # 'dev plugin pull [plugin ...]' — git pull the plugin checkouts (all, or the given ones).
    plugin_pull = plugin_sub.add_parser(
        "pull",
